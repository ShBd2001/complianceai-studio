...
| SSO d'entreprise (SAML) | Non implémenté, plus annoncé nulle part (retiré de la page Tarifs). « Se connecter avec Google » (OIDC, voir `/auth/google/*`) couvre un besoin proche (pas de mot de passe supplémentaire) mais pas le vrai motif SAML : le provisioning/déprovisioning centralisé par l'annuaire d'un cabinet | Chantier non engagé, pas de calendrier ; demanderait un fournisseur tiers (Auth0/WorkOS/Okta) ou une implémentation SAML complète |
